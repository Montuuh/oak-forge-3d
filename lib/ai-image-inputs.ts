...
/** N3D → color. Subidas locales → forma. `scene` = estudio fijo (v7). */
export type ReferenceImageRole = "color" | "geometry" | "color_geometry" | "scene";
...
export function hasSceneReference(inputs: GenerationImageInputs): boolean {
    return inputs.references.some((ref) => ref.role === "scene");
}
